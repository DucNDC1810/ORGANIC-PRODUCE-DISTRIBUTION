...
class NewsService {
  ...
  async fetchAgricultureNews(maxResults: number = 10): Promise<NewsArticle[]> {
    try {
      ...
      // Build search query for agriculture-related news with comprehensive keywords
      const query = 
        'organic farming OR sustainable agriculture OR ' +
        'organic food OR farm-to-table OR ' +
        'organic produce OR eco-friendly farming OR ' +
        'clean eating OR farmers market OR ' +
      ...
      if (error.response) {
        ...
        const message = error.response.data?.errors?.[0] || error.response.data?.message || 'Failed to fetch news';
        ...
        throw new Error('Unable to reach GNews API. Please check your internet connection');
      } else {
